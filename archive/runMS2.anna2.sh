...
########################################################
   ...
   res=$?; if [ $res != 0 ]; then echo "** ERROR $res"; exit 1; fi
   ...
   java -Xmx20000M -jar $MSGFplus_JAR -d $FASTAFILE -s $1 -o ${MSGFOUT}.mzid -tda 0 $MSGFplus_PARAMSTR
   res=$?; if [ $res != 0 ]; then echo "** ERROR $res"; exit 1; fi
   java -Xmx20000M -cp $MSGFplus_JAR edu.ucsd.msjava.ui.MzIDToTsv -i ${MSGFOUT}.mzid -o $TBL -showQValue 1 -showDecoy 1 -unroll 0
   res=$?; if [ $res != 0 ]; then echo "** ERROR $res"; exit 1; fi
...
echo "All $INPUTFILE MSblender tasks completed successfully! `date`"
