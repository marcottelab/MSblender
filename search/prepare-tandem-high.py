...
usage_mesg = 'Usage: prepare-tandem-high.py <mzXML directory> <fasta_pro file>'
...
filename_default_xml = os.path.join(MSTB_HOME,'search','isb_default_input_native.xml')
...
for filename in os.listdir(mzXML_dir):
    if( not filename.upper().endswith('.MZML') ):
