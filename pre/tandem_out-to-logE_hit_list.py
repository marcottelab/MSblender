...
usage_mesg = 'Usage: %s <X!Tandem out file>' % (__file__)
...
# MSups_5ul.09112.09112.2	2	2113.017700	-0.000600	MDQTLAVYQQILTSMPSR
#   P41159|LEP_HUMAN_UPS|147|50000.0	-1	4.343800
...
f_out.write("#input : %s\n" % filename_in)
f_out.write("#Spectrum_id\tCharge\tPrecursorMass\tMassDiff\tPeptide\t"
            + "Protein\tMissedCleavages\tScore(-log10[E-value])\n")
for spectrum_id in PSM.keys():
    ...
    spectrum_str = '%s.%05d.%05d.%d' % (filename_base, spectrum_id,
                                        spectrum_id, charge)

    f_out.write("%s\t%s\t%f\t%f\t%s\t%s\t%d\t%f\n" %
                (spectrum_str, charge, precursor_mass, massdiff, best_peptide,
                 best_protein, missed_cleavages, math.log10(best_expect)*-1))
f_out.close()
