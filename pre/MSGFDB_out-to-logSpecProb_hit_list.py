...
f_out.write("#input : %s\n"%filename_in)
f_out.write("#Spectrum_id\tCharge\tNeutralMass\tPeptide\tProtein\tMissedCleavages\tScore(-log10[SpecProb])\n")
...
for line in f_in:
    ...
    f_out.write("%s\t%d\t%f\t%s\t%s\t-1\t%f\n"%(sp_id,charge,neutral_mass,pep_seq,prot_id,-1.0*math.log10(SpecProb)))
f_in.close()
f_out.close()
