...
f_out.write("#Spectrum_id\tCharge\tNeutralMass\tPeptide\tProtein\tMissedCleavages\tScore(Xcorr)\n")
for spectrum_id in PSM.keys():
    ...
    f_out.write("%s\t%s\t%f\t%s\t%s\t%d\t%f\n"%(spectrum_id,charge,neutral_mass,best_peptide,best_protein,missed_cleavages,best_xcorr))
f_out.close()
