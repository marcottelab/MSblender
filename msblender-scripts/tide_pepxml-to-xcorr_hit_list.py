...
for spectrum_id in PSM.keys():
    sp_tokens = spectrum_id.split('.')
    sp_tokens[0] = filename_base
    new_sp_id = '.'.join(sp_tokens)
    ...
    if( best_xcorr == 0):
        continue
    f_out.write("%s\t%s\t%f\t%f\t%s\t%s\t%d\t%f\n"%(new_sp_id,charge,precursor_mz,massdiff,best_peptide,best_protein,missed_cleavages,best_xcorr))
f_out.close()
